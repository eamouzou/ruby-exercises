...
class EnumerablesTest < Minitest::Test
  ...
  def test_words_with_no_vowels
    ...
    actual = words.map {|word| word.delete("aeiouy")}
  ...
  def test_has_at_least_one_zero
    ...
    numbers = [2, 0, 9, 3, 0, 1]
    actual = numbers.any? {|number| number == 0} # one? doesn't work here
    #because it's checking if there is only one 0
  ...
  def test_count_numbers_greater_than_17
    ...
    actual = numbers.count {|number| number > 17}
  ...
  def test_none_are_negative
    ...
    actual = numbers.none? {|number| number.negative?}
    assert true, actual
  end
  ...
  def test_remove_numbers_divisible_by_3
    ...
    actual = numbers.reject {|number| number % 3 == 0}
  ...
  def test_has_a_multi_word_phrase
    ...
    actual = phrases.any? {|phrase| phrase.length > 1}
    assert_equal true, actual
  end
  ...
  def test_pick_dinosaurs
    ...
    actual = animals.find_all {|animal| animal.end_with?("aurus")}
  ...
  def test_sort_by_distance
    ...
    actual = distances.sort_by {|distance| distance.to_i}
  ...
  def test_remove_dinosaurs
    ...
    animals = ["tyrannosaurus", "narwhal", "eel", "achillesaurus", "qingxiusaurus"]
    notasaurus = animals.reject {|animal| animal.end_with?("aurus")}
  ...
  def test_remove_hashes
    ...
    actual = elements.reject {|element| element.class == Hash}
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    ...
    actual = keywords.reduce("one fish two fish red fish blue fish") do |acc, keyword|
      acc.gsub(keyword, keyword.upcase) #returning acc after this line cause it to fail
      #because it was resetting the acc or the string
      end
  ...
  def test_subtract_smallest_values_from_100
    ...
    actual = elements.reduce(100) do |acc, element|
      acc - element.min
      # returning acc here causes it to fail
    end
  ...
  def test_one_double_digit_number
    ...
    actual = numbers.one? {|number| number.to_s.length == 2}
    assert actual
  end
