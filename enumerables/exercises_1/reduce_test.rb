...
class ReduceTest < Minitest::Test
  ...
  def test_subtract_list_of_numbers
    ...
    result = numbers.reduce(0) do |difference, number|
      difference = difference - number #or could just be difference - number
      # without returning difference below
      difference
    end
  ...
  def test_multiply_list_of_numbers
    ...
    numbers = [2, 3, 5, 7]
    # initial value is 1
    result = numbers.reduce(1) do |acc, number|
      acc = acc * number
      acc
    end
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    ...
    result = keywords.reduce('one fish two fish red fish blue fish') do |acc, keyword|
      acc = acc.gsub(keyword, keyword.upcase)
      acc
    end
  ...
  def test_subtract_smallest_values_from_100
    ...
    result = elements.reduce(100) do |acc, arr|
      smallest_value = arr.min
      acc = acc - smallest_value
      acc
    end
  ...
  def test_add_all_the_second_values_together
    ...
    # initial value is 0
    result = elements.reduce(0) do |acc, arr|
      second_value = arr[1]
      acc = acc + second_value
      acc
    end
