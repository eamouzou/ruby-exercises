...
class SelectPatternTest < Minitest::Test
  ...
  def test_pick_words_ending_in_e
    ...
    selected = []
    words.each do |word|
      selected << word if word[-1] == "e" #another way - word.end_with?("e")
    end
  ...
  def test_pick_words_ending_in_ing
    ...
    selected = []
    words.each do |word|
      selected << word if word[-3..-1] == "ing" #another way - word.end_with?("ing")
    end
  ...
  def test_pick_dinosaurs
    ...
    animals.each do |animal|
      dinosaurs << animal if animal[-5..-1] == "aurus" #another way - animal.include?("aurus")
    end
