...
class CountPatternTest < Minitest::Test
  ...
  def test_count_multiples_of_5
    ...
    tally = 0
    numbers.each do |number|
      tally += 1 if number % 5 == 0 # testing id numbers divided by 5 have
      #a medulo or remainder of 0
    end
    assert_equal 3, tally
  end
  ...
  def test_count_round_prices
    ...
    prices.each do |price|
      tally += 1 if price.to_s.end_with?("0") # could do price == price.round(1).to_i.to_f
    end
    assert_equal 2, tally
  end
